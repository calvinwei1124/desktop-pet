...
| 双击宠物 | 打开设置窗口 |
| 点击托盘 `Pet` | 切换角色 / 设置 / 退出 |
| 菜单栏 `🍅 24:30` | 番茄钟运行时实时显示剩余时间 |

## 番茄工作法（休息提醒）

内置番茄钟，帮你按「专注 25 分钟 → 休息 5 分钟」的节奏工作，每完成 4 个番茄来一轮长休息。

- **桌面徽标**：宠物头顶实时显示 `阶段 剩余时间 ●●●○`，工作时红色、休息时绿色。
- **休息提醒**：阶段切换时，宠物停下冒出 `💤`，并弹出 macOS 系统通知（如「🍅 工作完成！伸展一下休息几分钟~」）。
- **菜单栏计时**：托盘标题在运行时会变成 `🍅 24:30` 实时倒计时。
- **托盘控制**：点击托盘 → 「开始/暂停番茄钟」「重置番茄钟」。
...
- **开机自动启动**：登录系统后自动运行
- **🍅 番茄工作法**：启用开关；可分别设置工作时长（5–60 分）、短休息（1–15 分）、
  长休息（5–30 分）、长休息间隔（每 2–8 个番茄）；并可开关系统通知、以及
  「工作后自动开始休息 / 休息后自动开始工作」。
